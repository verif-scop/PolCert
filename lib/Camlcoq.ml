...
module Nat = struct
  ...
  let rec of_int64 n =
    assert (n >= 0L);
    if n = 0L then O else S (of_int64 (Int64.pred n))  
...
let extern_atom' atom = 
  coqstring_of_camlstring (extern_atom atom)

let varname_to_ident varname = 
  try
    Hashtbl.find atom_of_string (camlstring_of_coqstring varname)
  with Not_found ->
    let a = fresh_atom () in
    Hashtbl.add string_of_atom a (camlstring_of_coqstring varname);
    Hashtbl.add atom_of_string (camlstring_of_coqstring varname) a;
    a



(** check function `fresh_atom` above *)
let free_ident () = 
    let a = fresh_atom () in
    (* 
    Hashtbl.add atom_of_string s a;
...
let bind_ident_varname id_varname_list =
  List.map (fun (id, varname) -> 
      Hashtbl.add string_of_atom id (camlstring_of_coqstring varname);
      Hashtbl.add atom_of_string (camlstring_of_coqstring varname) id;
      id
    ) id_varname_list


let iterator_to_varname depth =
  coqstring_of_camlstring (Printf.sprintf "$i%d" (Nat.to_int depth))
