...
def run_check(name: str, command: list[str], out_dir: Path, timeout: int | None) -> CheckResult:
    ...
    try:
        ...
        stderr += f"\n[check-smoke] timeout after {timeout} seconds\n"
        returncode = 124
...
def base_checks(
    ...
    identity_composition_command = [
        sys.executable,
        "tools/regression/explore_identity_compositions.py",
        "--output-root",
    ...
    return [
        (
            ...
            [
                ...
                "-m",
                "py_compile",
                "tools/regression/run_regression_suite.py",
                "tools/regression/explore_codegen_gaps.py",
                "tools/regression/explore_flag_effects.py",
                "tools/regression/explore_identity_compositions.py",
                "tools/regression/explore_unrolljam_effect_corpus.py",
                "tools/regression/test_runner_timeout.py",
                "tools/regression/test_tiling_route_summary.py",
                "tools/regression/test_release_provenance.py",
                "tools/regression/test_unrolljam_route_guard.py",
                "tools/regression/generate_capability_matrix.py",
                "tools/regression/proof_report.py",
        ...
        (
            "regression-runner-timeout-unit",
            [
                sys.executable,
                "tools/regression/test_runner_timeout.py",
            ],
...
def main() -> int:
    ...
    ap.add_argument("--output-root", default="/tmp/polcert-check-smoke")
    ...
    if provenance_errors:
        ...
        (out_dir / "regression-results.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True)
        )
        for error in provenance_errors:
            print(f"[check-smoke] provenance error: {error}", file=sys.stderr)
        return 2

    # Smoke mode is for quick regression checks, not for exhaustive route search.
    ...
    (out_dir / "regression-results.json").write_text(json.dumps(summary, indent=2, sort_keys=True))
    print(f"[check-smoke] summary: {out_dir / 'regression-results.json'}")
