...
Definition unaryop_eq_dec: 
  forall op1 op2: unary_operation, {op1 = op2} + {op1 <> op2}.
  decide equality.
Defined.

Definition unaryop_eqb (op1 op2: unary_operation): bool  
  := unaryop_eq_dec op1 op2.

Lemma unaryop_eqb_eq:
  forall op1 op2, unaryop_eqb op1 op2 = true <-> op1 = op2.
Proof.
  intros. 
  split.
  {
    intro.
    destruct op1; destruct op2; try discriminate; trivial.
  }
  {
    intro.
    destruct op1; destruct op2; try discriminate; trivial.
...
Lemma unaryop_eqb_refl: 
  forall op, unaryop_eqb op op = true.
Proof.
  intros. eapply unaryop_eqb_eq; trivial.
Qed.

Definition biop_eq_dec: 
forall op1 op2: binary_operation, {op1 = op2} + {op1 <> op2}.
decide equality.
Defined.

Definition biop_eqb (op1 op2: binary_operation): bool  
  := biop_eq_dec op1 op2.

Lemma biop_eqb_eq: 
  forall op1 op2, biop_eqb op1 op2 = true <-> op1 = op2.
Proof.
  intros. 
  split.
  {
...
Lemma biop_eqb_refl:
  forall op, biop_eqb op op = true.
Proof.
  intros. eapply biop_eqb_eq; trivial.
Qed.
...
(* for affine expression evaluation *)
(* Definition sem_add_ma (v1:val) (t1:type) (v2:val) (t2:type): option val :=
  match classify_add t1 t2 with
  | add_case_pi ty si =>             (**r pointer plus integer *)
      sem_add_ptr_int cenv ty si v1 v2
  | add_case_pl ty =>                (**r pointer plus long *)
...
(* Modified: not accessing memory (state) version *)
(* Only use in affine-related evaluation *)
(* Definition sem_unary_operation_ma
...
(* Modified version: not accessing state *)
(* only use for affine expression's evaluation *)
(* Definition sem_binary_operation_ma
  (op: binary_operation)
  (v1: val) (t1: type) (v2: val) (t2:type): option val :=
match op with
| Oadd => sem_add cenv v1 t1 v2 t2 m
